...
import { init as init_lc } from "license-checker"
let init = promisify(init_lc)
...
export async function getUserModulesInput (
  existingLicenses,
  existingModules
) {
...
export function pruneTreeByLicenses (name, node, invalidLicensedModules) {
  // interface Node { licenses: []; version: string }

  let prunedNode = {} as any
