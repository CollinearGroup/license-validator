...
function isModifyModulesPrompt(buffer) {
  return !!( // caste to boolean
    buffer.toString('utf8')
    .replace(/\n/g, '') // new lines cause problems
    .match(/.*Would you like to modify your approved module list\?.*\(Use arrow keys\)/m) // only works if initial prompt is unique
  )
}
...
function isAllowModulePrompt(buffer) {
  return !!( // caste to boolean
    buffer.toString('utf8')
    .replace(/\n/g, '') // new lines cause problems
    .match(/.*Would you like to allow this module anyway\?.*\(Use arrow keys\)/m) // only works if initial prompt is unique
  )
}
...
describe('integration test: validates bad files are cleanly handled', () => {
  ...
  it('should error on invalid yml', async () => {
    ...
    expect(stderr.toString('utf-8')).to.equal('Configuration file found but it is empty.\n')
  }).timeout(10000)
})
